...
import type { Tag, CreateTagDTO, UpdateTagDTO, ApiResponse } from "@/app/types";
import { csrfService } from "./csrfService";

class TagService {
  private baseUrl = `${API_CONFIG.BASE_URL}/tags`;
  ...
  async getAll(): Promise<Tag[]> {
    return this.request<Tag[]>(this.baseUrl, { method: "GET" });
  }

  async create(data: CreateTagDTO): Promise<Tag> {
    return this.request<Tag>(this.baseUrl, { method: "POST", body: JSON.stringify(data) });
  }

  async update(tagId: number, data: UpdateTagDTO): Promise<Tag> {
    return this.request<Tag>(`${this.baseUrl}/${tagId}`, { method: "PUT", body: JSON.stringify(data) });
  }

  async delete(tagId: number): Promise<void> {
    await this.request<{ deleted: boolean }>(`${this.baseUrl}/${tagId}`, { method: "DELETE" });
  }

  async assignToLink(linkId: number, tagIds: number[]): Promise<void> {
    await this.request<unknown>(`${this.baseUrl}/link/${linkId}`, { method: "PUT", body: JSON.stringify({ tagIds }) });
  }
}
